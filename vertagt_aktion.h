...
namespace vertagt
{
	...
	{
	private:
	    T p_obj; // Eklenmesi gereken obje

	public:
	    VPushFront(std::list<T>& liste, T obj)
	        : VAktion<T>(liste), p_obj(std::move(obj)) {}

	    virtual ~VPushFront() = default;

	    void vAusfuehren() override {
	        VAktion<T>::p_pListe.push_front(std::move(p_obj)); // Listeye başa ekle
	    }
	};
	...
	public:
		VErase() = default;
		 // Constructor: Liste referansı ve silinecek eleman iterator'u alır
		VErase(std::list<T>& liste, iterator it) : VAktion<T>(liste), p_it(it) {}
		virtual ~VErase() = default;
		void vAusfuehren() override
		{
			VAktion<T>::p_pListe.erase(p_it); // Liste'den belirtilen iterator'u sil
		}
	};
}  // namespace vertagt
